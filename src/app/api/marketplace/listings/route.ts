...
import { attachHandles, getHandleFromDID, getIndexedFeed } from '@/lib/server/browse-feed';
...
export async function GET(request: Request) {
    try {
        ...
        if (useAppView) {
            ...
            // served from cache and refreshed behind the response. Returns null
            // on any failure, which drops through to fan-out below rather than
            // failing the request — an index that is down must not mean a
            // marketplace that is down.
            const feed = await getIndexedFeed();
            if (feed) {
                return NextResponse.json({
                    listings: feed.listings,
                    count: feed.listings.length,
                    source: feed.source,
                    ...(feed.ageMs !== undefined ? { ageMs: feed.ageMs } : {}),
                });
            }
            // Only when the path was forced, so a normal fallback stays silent
            // rather than leaking internals into every response. Re-queried
            // directly because getIndexedFeed reports failure as a plain null.
            if (forced === 'appview') {
                const { error, url } = await fetchListingsFromAppView({ limit: 1 });
